...
* *hms-lambda-func: *an example Lambda function, where
    ...
    * *ThriftHiveMetaStoreClient*: a thrift client to communicate with hive metastore. This client is written for Hive 2.3.0 and Hive 3.1.x. For other hive versions, customer might need to update this class to make sure the response objects are compatible.
    ...
    * hms.properties: Lambda function configuration. Most likely customer only need to update the following properties
      ...
      * To use SSL connections when using higher EMR versions with in-transit encryption enabled, the connector needs access to a truststore that contains a certificate that the EMR cluster can verify, along with a password to access that truststore. Set the following lambda variables in the connector to use SSL connections -
        * "HMS_USE_SSL" lambda variable for hms property "hive.metastore.use.SSL": enables SSL connections to the Hive Metastore. When false (default), the thrift client attempts plain socket connection. When set to true, client attempts SSL connections. If using EMR clusters with hive versions 3.1.x, and enabling in transit encryption. Can be set through lambda environment variable "HMS_USE_SSL" as true.
        * "HMS_SSL_TRUSTSTORE_PATH" lambda variable for hms property "hive.metastore.ssl.truststore.path": truststore path where the certificate needed for an SSL connections are stored. Can be set through lambda environment variable "HMS_SSL_TRUSTSTORE_PATH" by the customers, needs to be an s3 path or a local lambda path.
        * "HMS_SSL_TRUSTSTORE_PASSWORD" lambda variable for hms property "hive.metastore.ssl.truststore.password": password to access the truststore to retrieve certificate for SSL connection. Can be set through lambda environment variable "HMS_SSL_TRUSTSTORE_PASSWORD".
        * The above properties could be overridden by Lambda environment variables (https://docs.aws.amazon.com/lambda/latest/dg/env_variables.html) so that customer don’t need to recompile the source code for different Lambda functions with different properties.
