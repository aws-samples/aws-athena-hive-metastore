...
* *hms-lambda-func: *an example Lambda function, where
    ...
    * *ThriftHiveMetaStoreClient*: a thrift client to communicate with hive metastore. This client is written for Hive 2.3.0 and Hive 3.1.x with automatic SSL fallback for newer versions. For other hive versions, customer might need to update this class to make sure the response objects are compatible.
    ...
    * hms.properties: Lambda function configuration. Most likely customer only need to update the following properties
        ...
        * hive.metastore.use.SSL: enables SSL connections to the Hive Metastore. When false (default), the client attempts plain socket connection first, then falls back to SSL if needed. Set to true to force SSL connections.
        * hive.metastore.ssl.truststore.path: truststore path where the certificate needed for an SSL connections are stored. When not specified, the client automatically retrieves certificates from the EMR cluster and creates a truststore.
        * hive.metastore.ssl.truststore.password: password to access the truststore to retrieve certificate for SSL connection
